    ...
    return (
        <form onSubmit={onSubmitHandler} className="flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll">
            <div className="bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded">
                ...
                <input type="text" placeholder="Type Here" required onChange={(e) => setTitle(e.target.value)}  value={title}
                className="w-full max-w-lg mt-2 p-2 border border-gray-300 outline-none rounded" />

                <p className="mt-4">Sub title</p>
                <input type="text" placeholder="Type Here" required onChange={(e) => setSubTitle(e.target.value)}  value={subTitle}
                className="w-full max-w-lg mt-2 p-2 border border-gray-300 outline-none rounded" />
                ...
                <div className="max-w-lg h-74 pb-16 sm:pb-10 pt-2 relative">
                    ...
                    <button type="button" disabled={loading} onClick={generateContent}
                ...
                type="submit" className="mt-8 w-40 h-10 bg-primary text-white rounded cursor-pointer text-sm">{isAdding ? "Adding..." : "Add Blog"}</button>
            </div>
        </form>
    )
}
